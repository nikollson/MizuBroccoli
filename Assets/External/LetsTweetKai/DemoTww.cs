...
public class DemoTww : MonoBehaviour 
{
    ...
    void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(0, 200, 200, 20), "Reset"))
        {

            PlayerPrefs.SetString(PLAYER_PREFS_TWITTER_USER_ID, "");
            PlayerPrefs.SetString(PLAYER_PREFS_TWITTER_USER_SCREEN_NAME, "");
            PlayerPrefs.SetString(PLAYER_PREFS_TWITTER_USER_TOKEN, "");
            PlayerPrefs.SetString(PLAYER_PREFS_TWITTER_USER_TOKEN_SECRET, "");
        }
        if (GUI.Button(new Rect(0, 250, 200, 20), "Upmedia"))
        {
            StartCoroutine(Twitter.API.PostMedia(sprite.texture.EncodeToPNG(), CONSUMER_KEY, CONSUMER_SECRET, m_AccessTokenResponse,
                new Twitter.PostMediaCollBack(this.OnUploadData)));
        }
    }
    ...
    void OnUploadData(bool success, string media_id)
    {
        print("OnUploadData - " + success + "  media : " + media_id);

        if (success)
        {
            StartCoroutine(Twitter.API.PostTweet(m_Tweet, media_id, CONSUMER_KEY, CONSUMER_SECRET, m_AccessTokenResponse,
                               new Twitter.PostTweetCallback(this.OnPostTweet)));
        }
    }
}
