...
namespace Library.EditablePolygon2D
{
    ...
    public class DelaunyTrianglation
    {
        struct Triangle
        {
            public int p1;
        ...
        bool IsCrossLine(Line A, Line B)
        {
            if (CCW(A.a, A.b, B.a) == CCW(A.a, A.b, B.b)) return false;
            if (CCW(B.a, B.b, A.a) == CCW(B.a, B.b, A.b)) return false;
            return true;
        }

        public int[] TriangulatePolygon_Track(Vector2[] XZofVertices)
        {
            int[] allTriangle = TriangulatePolygon(XZofVertices);
            int[] result = new int[(XZofVertices.Length - 2) * 3];

            int resCount = 0;
            for (int i = 0; i < allTriangle.Length; i += 3)
            {
                Vector2 center = Vector2.zero;
                for (int j = 0; j < 3; j++) center += XZofVertices[allTriangle[i + j]] / 3;


                float inf = (float)1E10;
                Line ray = new Line(center, center + new Vector2(inf, inf));

                int crossCount = 0;
                for (int j = 0; j < XZofVertices.Length; j++)
                {
                    Vector2 point1 = XZofVertices[j];
                    Vector2 point2 = XZofVertices[(j + 1) % XZofVertices.Length];
                    if (IsCrossLine(ray, new Line(point1, point2))) crossCount++;
                }
                if (crossCount % 2 == 1)
                {
                    for (int j = 0; j < 3; j++) result[resCount * 3 + j] = allTriangle[i + j];
                    resCount++;
                    if (resCount * 3 >= result.Length) break;
                }
            }
